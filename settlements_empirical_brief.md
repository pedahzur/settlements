...
- **Built-up-area scope:** `db/expansion.csv` contains the latest OCHA-derived built-up-area snapshot for 133 matched settlements. This is cross-sectional and does not measure historical physical expansion. A per-settlement time series would require reconstructing multiple polygon snapshots or historical imagery; construction starts remain available only in aggregate reports.
